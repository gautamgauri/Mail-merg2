...
export const enhanceEmailTemplate = async (
  ...
  if (!process.env.API_KEY) {
    ...
    // Here we simulate a helpful message if the key is missing.
    return `Error: API key not configured. Please ensure your environment is set up correctly. [Original text: ${originalBody}]`;
  }
  ...
  try {
    ...
    return `Error enhancing template. Please check the console for details.`;
  }
};
...
export const parseCommand = async (command: string, currentBody: string): Promise<GenerateContentResponse> => {
    ...
    try {
        ...
        throw new Error("Failed to understand the command.");
    }
};
